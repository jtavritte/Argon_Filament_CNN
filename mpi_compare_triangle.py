...
def Make_Fil(t):
    fil = Fil(imdat=cv2.imread(f'{shared_folder}/{shared_dict[t]}.tif', cv2.IMREAD_UNCHANGED))
...
def init_worker(folder,dictionary):
...
class Timestep_MPI():
    def __init__(self,folder='',step=1,model_file=[],label_file=[]):
    ...
    def run_split(self,step):
        ...
        #for ci,t_chunk in enumerate(time_split[0:1]):
            ...
            if extra:
                chunksize += 1
            #with Pool(os.cpu_count()-1,initializer=init_worker, initargs=(self.thresh,self.folder,self.t2f,model,labels)) as pool:
            #    all_fils_model = pool.imap_unordered(Make_Fil, t_chunk,chunksize=chunksize)
            #with Pool(os.cpu_count()-1,initializer=init_worker, initargs=(self.thresh,self.folder,self.t2f,model,labels)) as pool:
            with Pool(os.cpu_count()-1,initializer=init_worker, initargs=(self.folder,self.t2f)) as pool:
...
class Fil():
    ...
    def getLoc(self):
        ...
        c_area_roi = [c for c in contours if np.all(np.array(self.imgData.T.shape)-1-c.max(axis=0)[0] > 0) and np.all(c.min(axis=0)[0] > 0) and cv2.contourArea(c) > 200]
